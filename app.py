...
# Khởi tạo dữ liệu giả lập nghỉ phép nếu chưa có trong session_state
...
def render_quan_ly_cham_cong():
    ...
    with tab_cc1:
        ...
        dept_filter = col_m2.selectbox("Lọc theo Khoa / Phòng:", ["-- Tất cả Khoa / Phòng --"] + list(df_cb['khoa_phong'].dropna().unique()) if not df_cb.empty else ["-- Tất cả Khoa / Phòng --"])
        
        # Mẫu dữ liệu tổng hợp công
        data_cc = []
        if not df_cb.empty:
            for idx, r in df_cb.iterrows():
                data_cc.append({
                    "Mã Cán bộ": r['ma_can_bo'],
                    "Họ và Tên": r['ho_ten'],
                    "Khoa / Phòng": r['khoa_phong'],
                    "Công chuẩn": 22,
                    "Công thực tế": 21,
                    "Công trực 24h": 3,
                    "Phép năm": 1,
    ...
    with tab_cc2:
        ...
        with st.form("form_dang_ky_nghi"):
            ...
            loai_nghi = col_dk1.selectbox("Loại hình nghỉ (*):", ["Nghỉ phép năm", "Nghỉ bù trực", "Nghỉ ốm / BHXH", "Nghỉ thai sản", "Nghỉ việc riêng (Kết hôn, hiếu, hỷ)", "Nghỉ không hưởng lương"])
            ...
            if submit_don:
                if selected_cb_nghi == "-- Chọn cán bộ nghỉ --" or not ly_do_nghi.strip():
                    ...
                    st.session_state['don_nghi_phep'] = pd.concat([st.session_state['don_nghi_phep'], pd.DataFrame([new_don])], ignore_index=True)
...
# ---------------------------------------------------------
# 7. KHAI BÁO SIDEBAR & ĐIỀU HƯỚNG ROUTING THÔNG MINH
# ---------------------------------------------------------
def render_dashboard():
    ...
    elif "Quản lý Chấm công & Ngày nghỉ" in menu_choice:
