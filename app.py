...
with tab1:
    ...
    if df.empty:
        st.info("Chưa có dữ liệu nhân sự trong CSDL. Bạn có thể chuyển sang tab 'Tải File Mẫu & Nhập Excel' hoặc 'Thêm / Sửa Nhân sự' để cập nhật.")
    else:
        st.info(f"📊 **Tổng số nhân sự hiện có trong cơ sở dữ liệu:** **{len(df)}** cán bộ/nhân viên.")
...
# ==========================================
# TAB 3: TẢI FILE MẪU & NHẬP EXCEL
# ==========================================
with tab3:
    st.markdown("##### 📤 **2. Tải lên file Excel theo mẫu 2C-BNV**")
    uploaded_file = st.file_uploader("Chọn file Excel (.xlsx) chuẩn 2C-BNV", type=["xlsx", "xls"])
    
    if uploaded_file is not None:
        try:
            preview_df = pd.read_excel(uploaded_file)
            st.markdown(f"**Xem trước dữ liệu (Tổng số dòng: {len(preview_df)}):**")
            ...
            if st.button("🚀 Xác nhận Upload dữ liệu vào Hệ thống", use_container_width=True):
                ...
                with engine.connect() as conn:
                    for idx, row in preview_df.iterrows():
                        try:
                            mcb = str(row.get('Ma_Cb', row.get('Ma_Can_Bo', f'CB{idx}'))).strip()
                            hoten = str(row.get('Ten_Dvi_Khis', row.get('Ho_Ten', 'Chưa có tên'))).strip()
                            ngaysinh = str(row.get('Ngay_Sinh', '1990-01-01'))
                            cccd = str(row.get('So_CCCD', ''))
                            chucdanh = str(row.get('Chuc_Vu', row.get('Chuc_Danh', '')))
                            khoaphong = str(row.get('Khoa_Phong', ''))
                            trinhdo = str(row.get('Trinh_Do', ''))
                            sdt = str(row.get('Dien_Thoai', ''))
                            ...
                            """)
                            conn.execute(query_insert, {
                                "ma_cb": mcb, "ho_ten": hoten, "ngay_sinh": ngaysinh,
                                "so_cccd": cccd, "chuc_danh": chucdanh, "khoa_phong": khoaphong,
                                "trinh_do": trinh_do, "sdt": sdt, "email": email
                            })
                            conn.commit()
                            success_count += 1
                            ...
                            error_count += 1
                            error_details.append(f"Dòng {idx+1}: {str(row_err)}")
                ...
                st.markdown("---")
                if error_count == 0:
                    st.success(f"✅ **Đã hoàn thành upload dữ liệu thành công!** Đã nhập thành công **{success_count}** / {len(preview_df)} bản ghi.")
                else:
                    st.warning(f"⚠️ **Quá trình upload hoàn thành có lỗi:** Thành công: {success_count} bản ghi | Lỗi: {error_count} bản ghi.")
                    with st.expander("Xem chi tiết lỗi"):
                        for err in error_details[:10]:
                            st.text(err)
                
                updated_df = load_data()
                st.info(f"📊 **Tổng số nhân sự hiện có trong cơ sở dữ liệu:** **{len(updated_df)}** cán bộ/nhân viên.")
                st.balloons()
...
# ==========================================
with tab4:
    st.markdown("##### 📊 **Thống kê tổng quan nhân sự & Biểu đồ cảnh báo**")
    if df.empty:
        ...
        if 'khoa_phong' in df.columns:
            st.markdown("##### 🏥 **Biểu đồ phân bổ nhân sự theo Khoa / Phòng**")
            khoa_counts = df['khoa_phong'].value_counts().reset_index()
            khoa_counts.columns = ['Khoa / Phòng', 'Số lượng']
            st.dataframe(khoa_counts, use_container_width=True, hide_index=True)
            st.bar_chart(khoa_counts.set_index('Khoa / Phòng'))

# ==========================================
# TAB 5: XUẤT DATA EXCEL
# ==========================================
with tab5:
